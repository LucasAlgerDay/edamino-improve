...
class Bot:
    ...
    __slots__ = ('email', 'password', 'prefix', 'loop', 'sid', 'uid',
                 'timestamp', 'ws', 'client', 'futures', 'proxy', 'deviceId')
    ...
    def get_context(self, client: Client, msg: Message, ws):
        client_context = Client(session=client.session,
                                device_id=client.device_id if self.deviceId is None else self.deviceId,
                                com_id=msg.ndcId,
                                proxy=self.proxy)
        ...
        try:
            ...
            if self.sid is None:
                login = self.loop.run_until_complete(
                    self.client.login(self.email, self.password, self.deviceId))
                self.sid = login.sid
                self.uid = login.auid
